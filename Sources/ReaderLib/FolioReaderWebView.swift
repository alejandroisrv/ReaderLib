...
open class FolioReaderWebView: WKWebView {
    ...
    @objc func share(_ sender: UIMenuController) {
        ...
        let shareImage = UIAlertAction(title: self.readerConfig.localizedShareImageQuote, style: .default, handler: { (action) -> Void in
            if self.isShare {
                self.js("getHighlightContent()") { results in
                   if let textToShare = results {
                        self.folioReader.readerCenter?.presentQuoteShare(textToShare)
                   }
                }
            } else {
                self.js("getSelectedText()") { results in
                    if let textToShare = results {
                        self.folioReader.readerCenter?.presentQuoteShare(textToShare)
    ...
    func remove(_ sender: UIMenuController?) {
        ...
        js("removeThisHighlight()") { results in
            guard let removedId = results else { return }
    ...
    @objc func highlight(_ sender: UIMenuController?) {
        js("highlightString('\(HighlightStyle.classForStyle(self.folioReader.currentHighlightStyle))')") { highlightResults in
            
            guard let highlightAndReturn = highlightResults else { return }
            ...
            do {
                ...
                self.js("getHTML()"){ resultsHtml in
    ...
    @objc func highlightWithNote(_ sender: UIMenuController?) {
        var highlightAndReturn : String?
        
        self.js("highlightStringWithNote('\(HighlightStyle.classForStyle(self.folioReader.currentHighlightStyle))')") { results in
            highlightAndReturn = results
        }
        ...
        do {
            ...
            js("getHTML()") { results in
                
                guard let html = results else { return }
    ...
    @objc func updateHighlightNote (_ sender: UIMenuController?) {
        ...
        js("getHighlightId()") { results in
            
            guard let highlightId = results else { return }
    ...
    @objc func define(_ sender: UIMenuController?) {
        ...
        js("getSelectedText()") { results in
            textSelected = results
        }

        guard let selectedText = textSelected else { return }
    ...
    func changeHighlightStyle(_ sender: UIMenuController?, style: HighlightStyle) {
        ...
        js("setHighlightStyle('\(HighlightStyle.classForStyle(style.rawValue))')") { results in
            if let updateId = results {
